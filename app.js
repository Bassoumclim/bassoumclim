...
function clientPage(name){
  ...
  if(name==='invoices')return `${common}<div class="contentPanel"><div class="invoiceNote">ClimExpress ne collecte pas les paiements. Le règlement de la prestation se fait directement entre le client et le technicien.</div>${i.filter(x=>x.status==='completed').map(iv=>`<div class="listRow"><div><strong>Intervention · ${fmtDate(iv.completed_at||iv.created_at)}</strong><small>Prestation terminée · Paiement direct au technicien</small></div><strong>${money(iv.final_amount)}</strong></div>`).join('')||empty('Aucune intervention terminée pour le moment.')}</div>`;
