...
async function toggleAvailability(){if(!user||!techProfile)return alert('Profil technicien non chargé.');const next=!techProfile.is_available;const {error}=await sb.from('technicians').update({is_available:next}).eq('id',user.id);if(error)return alert('Impossible de modifier la disponibilité : '+error.message);await render()}
async function editProfile(){if(!user||!techProfile)return alert('Profil technicien non chargé.');let specialty=prompt('Spécialité',techProfile.specialty||'');if(specialty===null)return;let area=prompt('Zone d’intervention',techProfile.service_area||'');if(area===null)return;const {error}=await sb.from('technicians').update({specialty,service_area:area}).eq('id',user.id);if(error)return alert('Impossible de modifier le profil : '+error.message);await render()}
