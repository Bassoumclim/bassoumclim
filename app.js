...
async function signup(e){
  ...
  try{
    ...
    // Create database profiles only when Supabase has returned a real session.
    // If email confirmation is enabled, the user must first confirm and log in.
    if(data.session){
...
async function ensureProfile(u){
  if(!u||!sb)return {ok:false,error:new Error('Utilisateur ou client Supabase absent.')};
  ...
  try{
    const {data:existing,error:readError}=await sb.from('profiles').select('id').eq('id',u.id).maybeSingle();
    if(readError) throw readError;
    if(!existing){
      const {error:insertError}=await sb.from('profiles').insert(profileData);
      if(insertError) throw insertError;
    }
    if(m.role==='technicien'){
      const {data:existingTech,error:techReadError}=await sb.from('technicians').select('id').eq('id',u.id).maybeSingle();
      if(techReadError) throw techReadError;
      if(!existingTech){
        const {error:techInsertError}=await sb.from('technicians').insert({
          id:u.id,specialty:m.specialty||'Installation et entretien clim',
          service_area:m.service_area||'',is_verified:false,is_available:false
        });
