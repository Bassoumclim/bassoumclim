...
function setupAuthListener(){ if(!createSupabase()) return; function setupAuthListener(){ if(!createSupabase()) return; sb.auth.onAuthStateChange(()=>setTimeout(init,0)); } }
...
async function startApp(){
  if(!createSupabase()){ setTimeout(startApp,100); return; }
  setupAuthListener();
  await async function startApp(){if(!createSupabase()){setTimeout(startApp,100);return}setupAuthListener();await init()}
startApp();
}
startApp();
