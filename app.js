...
async function signup(e){
  ...
  try{
    ...
    // IMPORTANT : ne pas écrire dans profiles/technicians ici si Supabase
    // n'a pas encore fourni de session (cas fréquent avec la confirmation e-mail).
    // La création des profils sera faite automatiquement après connexion dans init().
    if(data.session){
      user=data.session.user;
      const profileResult=await ensureProfile(user);
      if(!profileResult.ok){
        console.error('Création du profil après inscription :',profileResult.error);
        msg('signupMsg','Compte créé, mais le profil n’a pas pu être préparé : '+(profileResult.error?.message||'erreur inconnue')+'',true);
        return;
      }
      msg('signupMsg',role==='technicien'
        ? '✓ Compte technicien créé et profil préparé. Vous pouvez accéder à votre espace.'
        : '✓ Compte créé et profil préparé.');
      setTimeout(()=>modal(false),700);
    }else{
      msg('signupMsg','✓ Compte créé. Vérifiez votre e-mail pour confirmer votre adresse, puis connectez-vous.',false);
    }
  }catch(err){
...
async function ensureProfile(u){
  if(!u||!sb) return {ok:false,error:new Error('Session Supabase absente.')};
  const m=u.user_metadata||{};
  const role=m.role==='technicien'?'technicien':'client';
  const profileData={id:u.id,role,first_name:m.first_name||'',last_name:m.last_name||'',phone:m.phone||''};
  try{
    const pr=await sb.from('profiles').select('id').eq('id',u.id).maybeSingle();
    if(pr.error) throw pr.error;
    if(!pr.data){
      const ins=await sb.from('profiles').insert(profileData).select('id').maybeSingle();
      if(ins.error) throw ins.error;
    }
    if(role==='technicien'){
      const tr=await sb.from('technicians').select('id').eq('id',u.id).maybeSingle();
      if(tr.error) throw tr.error;
      if(!tr.data){
        const tins=await sb.from('technicians').insert({
          id:u.id,
...
async function init(){
  ...
  try{
    ...
    const profileResult=await ensureProfile(user);
    if(!profileResult.ok) throw profileResult.error||new Error('Impossible de préparer votre profil.');
