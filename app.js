...
function clientPage(name){
  ...
  if(name==='profile')return `${common}<div class="contentPanel profilePanel"><div class="bigAvatar">${profile?.avatar_url?`<img src="${esc(profile.avatar_url)}" alt="Photo de profil" style="width:100%;height:100%;object-fit:cover;border-radius:50%">`:initials()}</div><div class="profileData"><div><span>Prénom</span><strong>${esc(profile?.first_name||'—')}</strong></div><div><span>Nom</span><strong>${esc(profile?.last_name||'—')}</strong></div><div><span>Téléphone</span><strong>${esc(profile?.phone||'—')}</strong></div><div><span>WhatsApp</span><strong>${esc(profile?.whatsapp||profile?.phone||'—')}</strong></div><div><span>Adresse</span><strong>${esc([profile?.neighborhood,profile?.city,profile?.address].filter(Boolean).join(', ')||'—')}</strong></div><div><span>Date de naissance</span><strong>${esc(profile?.birth_date?fmtDate(profile.birth_date):'—')}</strong></div><div><span>Genre</span><strong>${esc(profile?.gender||'—')}</strong></div><div><span>E-mail</span><strong>${esc(user?.email||'—')}</strong></div><div><span>Présentation</span><strong>${esc(profile?.bio||'—')}</strong></div></div><button class="secondaryBtn" data-action="edit-profile">Modifier mon profil</button></div>`;
...
async function loadTech(){
  ...
  const [rq,mq,iv,nf]=await Promise.all([
    ...
    sb.from('requests').select('id,client_id,service_type,description,address,preferred_date,budget,status,technician_id,created_at,contact_phone').eq('technician_id',user.id).order('created_at',{ascending:false}),
...
function techNav(key,label){return `<button class="sideLink ${activeTech===key?'active':''}" data-space="technicien" data-space-nav="${key}">${label}</button>`}
...
async function saveProfile(fields){
  const payload={first_name:fields.first_name?.trim()||'',last_name:fields.last_name?.trim()||'',phone:fields.phone?.trim()||'',whatsapp:fields.whatsapp?.trim()||'',address:fields.address?.trim()||'',city:fields.city?.trim()||'',neighborhood:fields.neighborhood?.trim()||'',birth_date:fields.birth_date||null,gender:fields.gender||'',bio:fields.bio?.trim()||'',updated_at:new Date().toISOString()};
  if(!payload.phone)throw new Error('Le téléphone est obligatoire.');
  const {error}=await sb.from('profiles').update(payload).eq('id',user.id);if(error)throw error;
  profile={...profile,...payload};
  if(profile.role==='technicien'){
    const {error:e}=await sb.from('technicians').update({specialty:fields.specialty?.trim()||''}).eq('id',user.id);if(e)throw e;
    techProfile={...techProfile,specialty:fields.specialty?.trim()||''};
  }
}
async function uploadAvatar(input){
  ...
  const {data}=sb.storage.from('profile-avatars').getPublicUrl(path);const avatar_url=data.publicUrl+`?v=${Date.now()}`;
  const {error:e}=await sb.from('profiles').update({avatar_url,updated_at:new Date().toISOString()}).eq('id',user.id);if(e)return alert(e.message);
  profile={...profile,avatar_url};await renderSpace();
}
async function editProfile(){
  const p=profile||{},t=techProfile||{};
  $('dashContent').innerHTML=`<div class="spacePageHead"><div><p class="eyebrow">MON PROFIL</p><h1>Modifier mon profil</h1><p>Renseignez vos informations pour avoir un profil complet.</p></div></div><div class="contentPanel"><div class="profileEdit"><div class="avatarEditor"><div class="avatarPreview">${p.avatar_url?`<img src="${esc(p.avatar_url)}" alt="Photo de profil">`:initials()}</div><label class="secondaryBtn">📷 Ajouter / changer la photo<input id="avatarInput" type="file" accept="image/jpeg,image/png,image/webp" hidden></label><small>JPG, PNG ou WEBP · 5 Mo maximum</small></div><form id="profileEditForm" class="profileForm"><div class="profileFormGrid"><label>Prénom<input name="first_name" value="${esc(p.first_name||'')}" required></label><label>Nom<input name="last_name" value="${esc(p.last_name||'')}"></label><label>Téléphone<input name="phone" value="${esc(p.phone||'')}" required></label><label>WhatsApp<input name="whatsapp" value="${esc(p.whatsapp||p.phone||'')}"></label><label>Adresse<input name="address" value="${esc(p.address||'')}"></label><label>Ville<input name="city" value="${esc(p.city||'')}" placeholder="Thiès, Dakar…"></label><label>Quartier<input name="neighborhood" value="${esc(p.neighborhood||'')}"></label><label>Date de naissance<input name="birth_date" type="date" value="${esc(p.birth_date||'')}"></label><label>Genre<select name="gender"><option value="">Non renseigné</option><option value="homme" ${p.gender==='homme'?'selected':''}>Homme</option><option value="femme" ${p.gender==='femme'?'selected':''}>Femme</option><option value="autre" ${p.gender==='autre'?'selected':''}>Autre</option></select></label>${p.role==='technicien'?`<label>Spécialité<input name="specialty" value="${esc(t.specialty||'')}" placeholder="Installation, entretien, réparation…"></label>`:''}</div><label>Présentation / informations complémentaires<textarea name="bio" rows="5" placeholder="Présentez-vous brièvement…">${esc(p.bio||'')}</textarea><button class="primary" type="submit">Enregistrer les modifications</button><p id="profileMsg" class="formMsg"></p></form></div></div>`;
  $('avatarInput')?.addEventListener('change',()=>uploadAvatar($('avatarInput')));
  $('profileEditForm')?.addEventListener('submit',async e=>{e.preventDefault();const btn=e.submitter;setBusy(btn,true);try{await saveProfile(Object.fromEntries(new FormData(e.currentTarget).entries()));alert('✓ Profil mis à jour.');await renderSpace();}catch(err){msg('profileMsg','Erreur : '+(err?.message||'Impossible de mettre à jour le profil.'),true)}finally{setBusy(btn,false)}});
}
...
async function init(){
  ...
  try{
    ...
    if(!user){document.body.classList.remove('client-mode','technician-mode','admin-mode');const h=$('publicHeader');h?.classList.remove('loggedIn');$('authBtn')?.classList.remove('hidden');$('userMenu')?.classList.add('hidden');$('dashboard')?.classList.add('hidden');return}
