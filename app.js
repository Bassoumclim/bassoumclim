...
async function startApp(){if(!ready()){setTimeout(startApp,250);return}initClient();bindStatic();if(sb)sb.auth.onAuthStateChange((event,session)=>{ if(event==='SIGNED_IN'||event==='SIGNED_OUT'||event==='TOKEN_REFRESHED') setTimeout(init,0); });await init()}
