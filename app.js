...
function bindStatic(){
  ...
  // Unified dashboard click handler: tabs + actions.
  // Bound to the stable dashboard container so it survives innerHTML refreshes.
  const dash=$('dashContent');
  if(dash){
    dash.onclick=async e=>{
      const tab=e.target.closest('[data-tab]');
      if(tab && dash.contains(tab)){
        e.preventDefault(); e.stopPropagation();
        techTab(tab.dataset.tab);
        return;
      }
      const action=e.target.closest('[data-action]');
      if(action && dash.contains(action)){
        e.preventDefault(); e.stopPropagation();
...
function clientRequest(x,q,i,rv){const z=q.find(a=>a.request_id===x.id&&a.status!=='rejected');const iv=i.find(a=>a.request_id===x.id);const review=iv&&rv.find(a=>a.intervention_id===iv.id);return `<div class="item"><div class="row"><b>${esc(x.service_type)}</b>${badge(x.status)}</div><p>${esc(x.description||'')}</p><small>📍 ${esc(x.address||'')} ${x.preferred_date?' · 📅 '+new Date(x.preferred_date).toLocaleString('fr-FR'):''}</small>
 ${z?`<div class="item"><div class="row"><b>Devis : ${money(z.amount)}</b>${badge(z.status)}</div><p>${esc(z.details||'')}</p>${z.status==='pending'?`<div class="actions"><button type="button" class="mini primary" data-action="accept-quote" data-id="${z.id}" data-request="${x.id}">Accepter le devis</button><button type="button" class="mini" data-action="reject-quote" data-id="${z.id}">Refuser</button></div>`:''}${z.status==='accepted'&&x.status!=='scheduled'?`<div class="scheduleForm"><label>Date et heure<input id="dt-${x.id}" type="datetime-local" required></label><button type="button" class="mini primary" data-action="schedule" data-id="${z.id}" data-request="${x.id}">Choisir le rendez-vous</button></div>`:''}</div>`:''}
 ${iv?`<div class="quoteBox"><div class="row"><b>Intervention</b>${badge(iv.status)}</div><p>${iv.scheduled_at?'Rendez-vous : '+new Date(iv.scheduled_at).toLocaleString('fr-FR'):''}</p>${iv.status==='completed'&&!review?`<div class="reviewBox"><b>Votre avis :</b><span class="stars" id="stars-${iv.id}">${[1,2,3,4,5].map(k=>`<button type="button" data-action="rate" data-id="${iv.id}" data-rating="${k}">☆</button>`).join('')}</span><input id="comment-${iv.id}" placeholder="Votre commentaire"><button type="button" class="mini primary" data-action="review" data-id="${iv.id}">Publier</button></div>`:review?`<p>⭐ ${review.rating}/5 · ${esc(review.comment||'Merci pour votre avis.')}</p>`:''}</div>`:''}</div>`}
...
function clientDashboard(requests,quotes,interventions,reviews){
  ...
  $('dashContent').innerHTML=`
    ...
    <div class="panel">
      <div class="sectionTitle"><div><h3>Notifications</h3></div><button type="button" class="mini" data-action="read-notifications">Marquer comme lues</button></div>
...
async function init(){
  ...
  try{
    ...
    if(!pr.data && meta.role==='technicien') profile.role='technicien';
    ...
    $('dashboard')?.classList.remove('hidden');
    $('dashContent').innerHTML=`<div class="panel"><h3>Impossible de charger votre espace</h3><p>${esc(err?.message||'Erreur inconnue')}</p><button type="button" class="mini primary" onclick="location.reload()">Réessayer</button></div>`;
  }
}
...
async function tech(){
 ...
 const [rq,mq,iq]=await Promise.all([
 ...
 $('dashContent').innerHTML=`<div class="techShell"><aside class="techSide"><div class="techBrand">❄️ BassoumClim <span style="opacity:.55">PRO</span></div><div class="techNav"><button type="button" class="active" data-tab="overview">📊 Vue d'ensemble</button><button data-tab="requests">📥 Demandes <b>(${a.length})</b></button><button data-tab="missions">🛠️ Mes missions</button><button data-tab="quotes">💰 Devis</button><button data-tab="history">📚 Historique</button><button data-tab="profile">👤 Mon profil</button></div></aside><div class="techMain"><div class="techTop"><div><div class="badge ok" style="display:inline-block;margin-bottom:8px">ESPACE TECHNICIEN</div><h3>Bonjour, ${display} 👋</h3><p>Gérez votre activité et vos interventions depuis cet espace professionnel.</p></div><div class="availability"><span class="dot ${t.is_available?'on':''}"></span><b>${t.is_available?'Disponible':'Indisponible'}</b><button type="button" class="mini ${t.is_available?'primary':''}" data-action="availability">${t.is_available?'Se rendre indisponible':'Activer'}</button></div></div>
 <section class="techSection active" data-section="overview"><div class="techKpis"><div class="kpi"><strong>${a.length}</strong><span>DEMANDES DISPONIBLES</span></div><div class="kpi"><strong>${active}</strong><span>MISSIONS ACTIVES</span></div><div class="kpi"><strong>${completed}</strong><span>INTERVENTIONS TERMINÉES</span></div><div class="kpi"><strong>${t.rating||'—'}</strong><span>NOTE MOYENNE</span></div></div><div class="panel"><div class="sectionTitle"><div><h3>Dernières demandes</h3><p>Les nouvelles missions en attente d'un technicien.</p></div><button type="button" class="mini" data-tab="requests">Voir toutes</button></div>${a.slice(0,3).map(requestCard).join('')||'<div class="emptyPro">Aucune nouvelle demande.</div>'}</div><div class="panel"><div class="sectionTitle"><div><h3>Mes missions en cours</h3></div><button type="button" class="mini" data-tab="missions">Gérer</button></div>${m.filter(x=>x.status!=='completed').slice(0,4).map(missionRow).join('')||'<div class="emptyPro">Aucune mission active.</div>'}</div></section>
 ...
 <section class="techSection" data-section="quotes"><div class="panel"><div class="sectionTitle"><div><h3>Gestion des devis</h3><p>Envoyez vos devis aux clients.</p></div></div>${m.filter(x=>['accepted','quoted','scheduled','in_progress'].includes(x.status)).map(x=>`<div class="requestCard"><div class="row"><div><h4>${esc(x.service_type)}</h4><small>${esc(x.address||'')}</small></div>${badge(x.status)}</div><p>${esc(x.description||'')}</p>${x.status==='accepted'?`<button type="button" class="mini primary" data-action="quote" data-id="${x.id}">+ Créer le devis</button>`:x.status==='quoted'?'<div class="quoteBox">💰 Devis envoyé · En attente du client.</div>':'<div class="quoteBox">✓ Intervention planifiée.</div>'}</div>`).join('')||'<div class="emptyPro">Aucun devis à gérer.</div>'}</div></section>
 <section class="techSection" data-section="history"><div class="panel"><div class="sectionTitle"><div><h3>Interventions</h3></div></div>${i.map(x=>`<div class="missionRow"><div><b>${esc(x.status==='completed'?'Intervention terminée':'Intervention')}</b><p>${x.completed_at?new Date(x.completed_at).toLocaleDateString('fr-FR'):x.scheduled_at?new Date(x.scheduled_at).toLocaleDateString('fr-FR'):''}</p></div><div>${badge(x.status)}</div><div><b>${money(x.final_amount)}</b></div><div>${x.status==='scheduled'?`<button type="button" class="mini" data-action="start-intervention" data-id="${x.id}">Démarrer</button>`:x.status==='in_progress'?`<button type="button" class="mini primary" data-action="complete-intervention" data-id="${x.id}">Terminer</button>`:''}</div></div>`).join('')||'<div class="emptyPro">Aucune intervention.</div>'}</div></section>
 <section class="techSection" data-section="profile"><div class="panel"><div class="sectionTitle"><div><h3>Mon profil professionnel</h3></div><button type="button" class="mini primary" data-action="edit-profile">Modifier</button></div><div class="profilePro"><div class="avatar">${esc(((profile?.first_name||'T')[0]+(profile?.last_name||'')).slice(0,2).toUpperCase())}</div><div><h3>${display}</h3><div class="profileRows"><div class="profileField"><small>SPÉCIALITÉ</small><b>${esc(t.specialty||'À renseigner')}</b></div><div class="profileField"><small>ZONE</small><b>${esc(t.service_area||'À renseigner')}</b></div><div class="profileField"><small>VÉRIFICATION</small><b>${t.is_verified?'✓ Profil vérifié':'En attente'}</b></div></div></div></div></div></section></div></div>`;
 document.querySelectorAll('.techNav [data-tab],.techTop [data-tab]').forEach(b=>b.onclick=()=>techTab(b.dataset.tab));
}
function requestCard(x){return `<div class="requestCard"><div class="row"><div><h4>${esc(x.service_type)}</h4><div class="meta"><span>📍 ${esc(x.address||'')}</span>${x.preferred_date?`<span>📅 ${new Date(x.preferred_date).toLocaleString('fr-FR')}</span>`:''}${x.budget?`<span>💰 ${money(x.budget)}</span>`:''}</div></div>${badge(x.status)}</div><p>${esc(x.description||'Aucune description')}</p><button type="button" class="mini primary" data-action="take" data-id="${x.id}">Accepter la mission →</button></div>`}
function missionRow(x){return `<div class="missionRow"><div><b>${esc(x.service_type)}</b><p>${esc(x.address||'')}</p></div><div>${badge(x.status)}</div><div><p>${x.preferred_date?'Souhait : '+new Date(x.preferred_date).toLocaleDateString('fr-FR'):''}</p></div><div>${x.status==='accepted'?`<button type="button" class="mini primary" data-action="quote" data-id="${x.id}">Faire un devis</button>`:x.status==='quoted'?'<span class="badge warn">Devis en attente</span>':''}</div></div>`}
