...
function globalClick(e){
  ...
  if(roleNav){e.preventDefault();const space=roleNav.dataset.space||(profile?.role==='technicien'?'technicien':'client');navigateSpace(space,roleNav.dataset.spaceNav);return}
...
async function createRequest(e){
  ...
  try{
    const payload={client_id:user.id,service_type:$('service')?.value||'installation',contact_phone:$('phone')?.value.trim()||'',address:$('address')?.value.trim()||'',preferred_date:$('date')?.value?new Date($('date').value).toISOString():null,budget:$('budget')?.value?Number($('budget').value):null,description:$('description')?.value.trim()||'',status:'pending'};
...
async function schedule(q,r){const el=$('dt-'+r);if(!el?.value)return alert('Choisissez une date et une heure.');const scheduled=new Date(el.value);if(Number.isNaN(scheduled.getTime())||scheduled.getTime()<Date.now())return alert('Choisissez une date et une heure valides dans le futur.');const {data,error}=await sb.rpc('schedule_intervention',{p_quote_id:q,p_request_id:r,p_scheduled_at:scheduled.toISOString()});if(error)return alert(error.message);if(!data)return alert('Impossible de planifier cette intervention.');alert('✓ Rendez-vous enregistré.');await renderSpace()}
...
async function startIntervention(id){const {data,error}=await sb.rpc('technician_start_intervention',{p_intervention_id:id});if(error)return alert(error.message);if(!data)return alert('Impossible de démarrer cette intervention.');await renderSpace()}
async function completeIntervention(id){const amount=Number(prompt('Montant final en FCFA'));if(!Number.isFinite(amount)||amount<0)return;const {data,error}=await sb.rpc('technician_complete_intervention',{p_intervention_id:id,p_final_amount:amount});if(error)return alert(error.message);if(!data)return alert('Impossible de terminer cette intervention.');await renderSpace()}
...
function applyRoleUI(role){
  ...
  if(!user){header.classList.remove('loggedIn');$('authBtn')?.classList.remove('hidden');$('userMenu')?.classList.add('hidden');return}
  document.body.classList.add(role==='technicien'?'technician-mode':role==='admin'?'admin-mode':'client-mode');
  header.classList.add('loggedIn');
