...
async function signup(e){
  ...
  try{
    ...
    /*
      IMPORTANT :
      Si la confirmation e-mail est activée, Supabase peut créer data.user
      sans créer data.session. Dans ce cas, il ne faut surtout pas essayer
      d'écrire dans profiles/technicians avec une requête non authentifiée.
      Le profil sera créé automatiquement après la première vraie connexion.
    */
    if(data.session){
      user=data.session.user;
      const profileResult=await ensureProfile(user);
      if(!profileResult.ok){
        console.warn('Création du profil après inscription :',profileResult.error);
      }
    }

    if(role==='technicien'){
      msg('signupMsg', data.session
        ? '✓ Compte technicien créé. Connexion réussie.'
...
async function ensureProfile(u){
  if(!u||!sb) return {ok:false,error:new Error('Utilisateur ou Supabase indisponible.')};

  const m=u.user_metadata||{};
  const role=m.role==='technicien'?'technicien':'client';

  try{
    // On vérifie d'abord si le profil existe pour éviter un upsert inutile.
    ...
    // Le profil technicien est créé uniquement avec une session authentifiée.
    if(role==='technicien'){
      const {data:existingTech,error:techReadError}=await sb
        .from('technicians')
        .select('id')
        .eq('id',u.id)
        .maybeSingle();

      if(techReadError) throw techReadError;

      if(!existingTech){
        const {error:techInsertError}=await sb.from('technicians').insert({
          id:u.id,
    ...
    console.warn('Impossible de créer/vérifier le profil :',err);
...
async function init(){
  ...
  try{
    ...
    /*
      Après une connexion réelle, on s'assure que profiles et technicians
      existent avant de charger le tableau de bord. C'est ce qui corrige le
      cas où l'inscription avait créé auth.users mais pas le profil technicien.
    */
    const profileResult=await ensureProfile(user);
    if(!profileResult.ok){
      throw profileResult.error||new Error('Impossible de préparer votre profil.');
    }
...
async function tech(){
 ...
 const [{data:a=[]},{data:m=[]},{data:i=[]}]=await Promise.all([sb.from('requests').select('*').is('technician_id',null).eq('status','pending').order('created_at',{ascending:false}),sb.from('requests').select('*').eq('technician_id',user.id).order('created_at',{ascending:false}),sb.from('interventions').select('*').eq('technician_id',user.id).order('created_at',{ascending:false})]);
 ...
 document.querySelectorAll('.techNav [data-tab],.techTop [data-tab]').forEach(b=>{b.onclick=null;b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();techTab(b.dataset.tab);});});
}
