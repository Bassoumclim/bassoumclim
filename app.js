...
let clientState={requests:[],quotes:[],interventions:[],reviews:[],notifications:[]};
...
function toggleTechFields(){const tech=$('roleSelect')?.value==='technicien';$('techFields')?.classList.toggle('hidden',!tech);if($('specialty'))$('specialty').required=tech}
...
function globalClick(e){
  ...
  if(roleNav){
    e.preventDefault();
    const space=roleNav.dataset.space || (roleNav.closest('.clientShell')?'client':roleNav.closest('.techShell')?'technicien':null);
    if(space) navigateSpace(space,roleNav.dataset.spaceNav);
    return;
  }
...
async function signup(e){
  ...
  const role=document.body.dataset.spaceOnly||$('roleSelect')?.value||'client',btn=e.submitter;setBusy(btn,true);msg('signupMsg','Création du compte…');
  ...
  if(role==='technicien'){meta.specialty=$('specialty')?.value.trim()||'Installation et entretien clim'}
  try{
...
async function ensureProfile(u){
  ...
  try{
    ...
    if(m.role==='technicien')await sb.from('technicians').upsert({id:u.id,specialty:m.specialty||'Installation et entretien clim',is_verified:false,is_available:false},{onConflict:'id'});
...
async function createRequest(e){
  ...
  try{
    ...
    const payload={client_id:user.id,service_type:['installation','entretien','reparation'].includes(service)?service:'installation',contact_phone:$('phone')?.value.trim()||'',address:$('address')?.value.trim()||'',preferred_date:$('date')?.value?new Date($('date').value).toISOString():null,budget:$('budget')?.value?Number($('budget').value):null,description:$('description')?.value.trim()||'',status:'pending'};
...
async function loadClient(){
  ...
  clientState={requests:rq.data||[],quotes:qt.data||[],interventions:ivs,reviews:rv.data||[],notifications:nf.data||[]};
}
function clientPage(name){
  const r=clientState.requests,q=clientState.quotes,i=clientState.interventions,n=clientState.notifications;
  const first=profile?.first_name||'Client';
  const common=`<div class="spacePageHead"><div><p class="eyebrow">ESPACE CLIENT</p><h1>${name==='dashboard'?`Bonjour, ${esc(first)} 👋`:esc(name)}</h1><p>${name==='dashboard'?'Bienvenue dans votre espace client KërClim.':'Retrouvez ici les informations essentielles de votre compte.'}</p></div></div>`;
  if(name==='dashboard'){
    ...
    return `${common}<div class="clientBookingHero"><div><span>Votre prochaine intervention</span><strong>${nextClientIntervention(i,r)}</strong><small>Gérez vos demandes, rendez-vous et devis depuis votre espace.</small></div><button class="primary" data-action="client-reserve">Réserver un service</button></div>
      <div class="statsGrid clientStatsGrid"><div class="statCard"><span>Réservations</span><strong>${r.length}</strong></div><div class="statCard"><span>Interventions</span><strong>${i.length}</strong></div><div class="statCard"><span>Terminées</span><strong>${i.filter(x=>x.status==='completed').length}</strong></div></div>
  ...
  if(name==='devices')return `${common}<div class="contentPanel emptyState"><div class="emptyIcon">❄</div><h2>Aucun appareil enregistré</h2><p>La gestion des appareils n’est pas encore alimentée par les données actuelles de KërClim.</p></div>`;
  if(name==='profile')return `${common}<div class="contentPanel profilePanel"><div class="bigAvatar">${profile?.avatar_url?`<img src="${esc(profile.avatar_url)}" alt="Photo" style="width:100%;height:100%;object-fit:cover;border-radius:50%">`:initials()}</div><div class="profileData"><div><span>Prénom</span><strong>${esc(profile?.first_name||'—')}</strong></div><div><span>Nom</span><strong>${esc(profile?.last_name||'—')}</strong></div><div><span>Téléphone</span><strong>${esc(profile?.phone||'—')}</strong></div><div><span>WhatsApp</span><strong>${esc(profile?.whatsapp||profile?.phone||'—')}</strong></div><div><span>Adresse</span><strong>${esc([profile?.neighborhood,profile?.city,profile?.address].filter(Boolean).join(', ')||'—')}</strong></div><div><span>E-mail</span><strong>${esc(user?.email||'—')}</strong></div></div><button class="secondaryBtn" data-action="edit-profile">Modifier</button></div>`;
...
function normalizeContactPhone(value){let v=String(value||'').trim().replace(/[^0-9+]/g,'');if(v.startsWith('00'))v='+'+v.slice(2);if(v.startsWith('+'))return v;if(v.length===9)return '+221'+v;return v;}
function whatsappPhone(value){return normalizeContactPhone(value).replace(/\D/g,'');}
function canContactRequest(x){return !!x?.technician_id&&['accepted','quoted','scheduled','in_progress','completed'].includes(x.status)}
function contactHost(id,label){return `<div class="contactHost" id="contact-host-${esc(id)}"><button class="secondaryBtn smallBtn" data-action="contact-request" data-id="${esc(id)}">📞 ${label}</button></div>`}
async function contactRequest(id){const host=$('contact-host-'+id);if(!host)return;host.innerHTML='<span class="contactLabel">Récupération des coordonnées…</span>';const {data,error}=await sb.rpc('get_request_contacts',{p_request_id:id});if(error){host.innerHTML='<span class="contactLabel">Coordonnées indisponibles.</span>';return alert(error.message)}const c=Array.isArray(data)?data[0]:data;const phone=normalizeContactPhone(c?.other_phone);if(!phone){host.innerHTML='<span class="contactLabel">Aucun numéro renseigné.</span>';return}const wa=whatsappPhone(phone);host.innerHTML=`<span class="contactLabel">${esc(c?.other_name||'Contact')}</span><div class="contactActions"><a class="secondaryBtn smallBtn" href="tel:${esc(phone)}">📞 Appeler</a><a class="secondaryBtn smallBtn" href="https://wa.me/${esc(wa)}" target="_blank" rel="noopener">💬 WhatsApp</a></div>`;}
function clientReservationDetail(x){const q=clientState.quotes.find(v=>v.request_id===x.id&&v.status!=='rejected'),iv=clientState.interventions.find(v=>v.request_id===x.id);const canCancel=['pending','quoted','accepted','scheduled'].includes(x.status);return `<div class="reservationCard"><div class="reservationTop"><div><strong>${esc(x.service_type)}</strong><small>${esc(x.address||'Adresse non renseignée')}</small></div>${badge(x.status)}</div><div class="reservationMeta"><span>Date : ${fmtDateTime(x.preferred_date)}</span><span>Téléphone : ${esc(x.contact_phone||'—')}</span></div><p>${esc(x.description||'')}</p>${q?`<div class="quoteInline"><strong>Devis : ${money(q.amount)}</strong>${badge(q.status)}${q.status==='pending'?`<div class="actions"><button class="primary smallBtn" data-action="accept-quote" data-id="${q.id}" data-request="${x.id}">Accepter</button><button class="secondaryBtn" data-action="reject-quote" data-id="${q.id}">Refuser</button></div>`:''}${q.status==='accepted'&&x.status!=='scheduled'?`<div class="scheduleInline"><input id="dt-${x.id}" type="datetime-local"><button class="primary smallBtn" data-action="schedule" data-id="${q.id}" data-request="${x.id}">Choisir le rendez-vous</button></div>`:''}</div>`:''}${iv?`<div class="quoteInline"><strong>Intervention</strong>${badge(iv.status)}<span>${fmtDateTime(iv.scheduled_at)}</span>${iv.status==='completed'&&!clientState.reviews.find(v=>v.intervention_id===iv.id)?reviewBox(iv.id):''}</div>`:''}${canContactRequest(x)?contactHost(x.id,'Contacter le technicien'):''}${canCancel?`<button class="secondaryBtn smallBtn cancelBtn" data-action="cancel-request" data-id="${x.id}">Annuler la réservation</button>`:''}</div>`}
...
async function loadTech(){
  ...
  const [rq,mq,iv,nf]=await Promise.all([
    sb.from('requests').select('id,client_id,technician_id,service_type,description,address,status,created_at,preferred_date,budget').is('technician_id',null).eq('status','pending').order('created_at',{ascending:false}),
...
function techPage(name){
  ...
  if(name==='profile')return `${common}<div class="contentPanel profilePanel"><div class="bigAvatar">${profile?.avatar_url?`<img src="${esc(profile.avatar_url)}" alt="Photo" style="width:100%;height:100%;object-fit:cover;border-radius:50%">`:initials()}</div><div class="profileData"><div><span>Nom</span><strong>${esc(fullName())}</strong></div><div><span>Téléphone</span><strong>${esc(profile?.phone||'—')}</strong></div><div><span>WhatsApp</span><strong>${esc(profile?.whatsapp||profile?.phone||'—')}</strong></div><div><span>Adresse</span><strong>${esc([profile?.neighborhood,profile?.city,profile?.address].filter(Boolean).join(', ')||'—')}</strong></div><div><span>Spécialité</span><strong>${esc(techProfile?.specialty||'—')}</strong></div><div><span>Vérification</span><strong>${techProfile?.is_verified?'Profil vérifié':'En attente de vérification'}</strong></div><div><span>E-mail</span><strong>${esc(user?.email||'—')}</strong></div></div><button class="secondaryBtn" data-action="edit-profile">Modifier</button></div>`;
...
async function createQuote(id){const amount=Number(prompt('Montant du devis en FCFA'));if(!Number.isFinite(amount)||amount<=0)return;const details=prompt('Détails du devis')||'';const {error}=await sb.rpc('technician_submit_quote',{p_request_id:id,p_amount:amount,p_details:details});if(error)return alert(error.message);alert('✓ Devis envoyé au client.');await renderSpace()}
async function acceptQuote(q,r){const {data,error}=await sb.rpc('client_accept_quote',{p_quote_id:q});if(error)return alert(error.message);if(!data)return alert('Ce devis n’est plus disponible.');await renderSpace()}
async function rejectQuote(id){const {data,error}=await sb.rpc('client_reject_quote',{p_quote_id:id});if(error)return alert(error.message);if(!data)return alert('Ce devis n’est plus disponible.');await renderSpace()}
async function schedule(q,r){const el=$('dt-'+r);if(!el?.value)return alert('Choisissez une date et une heure.');const scheduled=new Date(el.value);if(Number.isNaN(scheduled.getTime()))return alert('Date invalide.');const {data,error}=await sb.rpc('schedule_intervention',{p_quote_id:q,p_request_id:r,p_scheduled_at:scheduled.toISOString()});if(error)return alert(error.message);if(!data)return alert('Impossible de planifier le rendez-vous.');alert('✓ Rendez-vous enregistré.');await renderSpace()}
async function saveProfile(fields){
  const payload={first_name:fields.first_name,last_name:fields.last_name,phone:fields.phone,address:fields.address,city:fields.city,neighborhood:fields.neighborhood,whatsapp:fields.whatsapp,birth_date:fields.birth_date||null,gender:fields.gender,bio:fields.bio,updated_at:new Date().toISOString()};
  const {error}=await sb.from('profiles').update(payload).eq('id',user.id); if(error) throw error;
  profile={...profile,...payload};
  if(profile.role==='technicien'){
    const {error:e}=await sb.from('technicians').update({specialty:fields.specialty||techProfile?.specialty||''}).eq('id',user.id); if(e) throw e;
    techProfile={...techProfile,specialty:fields.specialty||techProfile?.specialty||''};
  }
}
async function uploadAvatar(input){
  const file=input.files?.[0]; if(!file||!user)return;
  if(!['image/jpeg','image/png','image/webp'].includes(file.type)) return alert('Photo JPG, PNG ou WEBP uniquement.');
  if(file.size>5*1024*1024)return alert('La photo doit faire 5 Mo maximum.');
  const ext=file.name.split('.').pop().toLowerCase(); const path=`${user.id}/avatar.${ext}`;
  const {error}=await sb.storage.from('profile-avatars').upload(path,file,{upsert:true,contentType:file.type}); if(error)return alert(error.message);
  const {data}=sb.storage.from('profile-avatars').getPublicUrl(path); const url=data.publicUrl+`?v=${Date.now()}`;
  const {error:e}=await sb.from('profiles').update({avatar_url:url,updated_at:new Date().toISOString()}).eq('id',user.id); if(e)return alert(e.message);
  profile={...profile,avatar_url:url}; await renderSpace();
}
function profileForm(role){
 const p=profile||{}, t=techProfile||{};
 return `<div class="contentPanel"><div class="panelHead"><div><h2>Modifier mon profil</h2><p>Renseignez les informations que vous souhaitez afficher et utiliser pour vous contacter.</p></div></div>
 <div class="profileEdit"><div class="avatarEditor"><img id="profileAvatarPreview" src="${esc(p.avatar_url||'')}" onerror="this.style.display='none'" alt="Photo de profil"><div class="bigAvatar">${initials()}</div><label class="secondaryBtn">📷 Ajouter / changer la photo<input id="avatarInput" type="file" accept="image/jpeg,image/png,image/webp" hidden></label><small>JPG, PNG ou WEBP · 5 Mo max.</small></div>
 <form id="profileEditForm" class="profileForm"><div><label>Prénom<input name="first_name" value="${esc(p.first_name||'')}" required></label><label>Nom<input name="last_name" value="${esc(p.last_name||'')}"></label></div><div><label>Téléphone<input name="phone" value="${esc(p.phone||'')}" required></label><label>WhatsApp<input name="whatsapp" value="${esc(p.whatsapp||p.phone||'')}"></label></div><div><label>Adresse<input name="address" value="${esc(p.address||'')}"></label><label>Ville<input name="city" value="${esc(p.city||'')}" placeholder="Thiès, Dakar…"></label></div><div><label>Quartier<input name="neighborhood" value="${esc(p.neighborhood||'')}"></label><label>Date de naissance<input name="birth_date" type="date" value="${esc(p.birth_date||'')}"></label></div><div><label>Genre<select name="gender"><option value="">Non renseigné</option><option ${p.gender==='homme'?'selected':''} value="homme">Homme</option><option ${p.gender==='femme'?'selected':''} value="femme">Femme</option><option ${p.gender==='autre'?'selected':''} value="autre">Autre</option></select></label>${role==='technicien'?`<label>Spécialité<input name="specialty" value="${esc(t.specialty||'')}" placeholder="Installation, entretien, réparation…"></label>`:''}</div><label>Présentation / informations complémentaires<textarea name="bio" placeholder="Quelques informations sur vous…">${esc(p.bio||'')}</textarea><button class="primary">Enregistrer les modifications</button><p id="profileMsg" class="formMsg"></p></form></div></div>`;
}
async function editProfile(){
  const role=profile?.role; $('dashContent').innerHTML=profileForm(role); const input=$('avatarInput'); if(input) input.addEventListener('change',()=>uploadAvatar(input)); const form=$('profileEditForm');
  form?.addEventListener('submit',async e=>{e.preventDefault();const f=new FormData(form);const fields=Object.fromEntries(f.entries());try{await saveProfile(fields);alert('✓ Profil mis à jour.');await renderSpace();}catch(err){msg('profileMsg','Erreur : '+err.message,true)}});
}
async function editProfileLegacy(){await editProfile()}
async function startIntervention(id){const {data,error}=await sb.rpc('technician_start_intervention',{p_intervention_id:id});if(error)return alert(error.message);if(!data)return alert('Cette intervention ne peut pas être démarrée.');await renderSpace()}
async function completeIntervention(id){const amount=Number(prompt('Montant final en FCFA'));if(!Number.isFinite(amount)||amount<0)return;const {data,error}=await sb.rpc('technician_complete_intervention',{p_intervention_id:id,p_final_amount:amount});if(error)return alert(error.message);if(!data)return alert('Cette intervention ne peut pas être terminée.');await renderSpace()}
...
async function handleAction(el){
  ...
  try{
    ...
    if(a==='cancel-request'){await cancelRequest(el.dataset.id);return}
    if(a==='schedule'){await schedule(el.dataset.id,el.dataset.request);return}
    if(a==='edit-profile'){await editProfile();return}
    if(a==='contact-request'){await contactRequest(el.dataset.id);return}
...
function applyRoleUI(role){
  document.body.classList.remove('client-mode','technician-mode','admin-mode');
  if(user && ['client','technicien','admin'].includes(role)) document.body.classList.add(role==='technicien'?'technician-mode':role==='admin'?'admin-mode':'client-mode');
...
async function init(){
  ...
  try{
    ...
    if(!user){applyRoleUI(null);$('dashboard')?.classList.add('hidden');if(document.body.dataset.spaceOnly) { modal(true); showLogin(); } return}
    ...
    applyRoleUI(profile.role); if(document.body.dataset.spaceOnly && profile.role!==document.body.dataset.spaceOnly){ $('dashboard')?.classList.remove('hidden'); $('dashContent').innerHTML=`<div class="accessDenied"><div class="accessIcon">🔒</div><h1>Espace ${document.body.dataset.spaceOnly==='client'?'Client':'Technicien'}</h1><p>Ce compte n’a pas accès à cet espace.</p><button class="primary" data-action="logout">Se déconnecter</button></div>`; return } $('dashboard')?.classList.remove('hidden');await renderSpace();
