...
async function signup(e){
  ...
  try{
    ...
    // Créer les profils uniquement lorsqu'une vraie session est disponible.
    if(data.session){
    ...
    if(data.session){
      msg('signupMsg',
        role==='technicien'
          ? '✓ Compte technicien créé. Vous êtes connecté.'
          : '✓ Compte créé. Vous êtes connecté.');
      setTimeout(()=>modal(false),700);
    }else{
      msg('signupMsg',
        role==='technicien'
          ? '✓ Compte technicien créé ! Allez confirmer votre e-mail dans votre boîte de réception, puis revenez vous connecter.'
          : '✓ Compte créé ! Allez confirmer votre e-mail dans votre boîte de réception, puis revenez vous connecter.');
    }
  }catch(err){
...
async function ensureProfile(u){
  if(!u||!sb)return {ok:false,error:new Error('Utilisateur non authentifié.')};
  const m=u.user_metadata||{};
  const profileData={
    id:u.id,
    role:m.role==='technicien'?'technicien':'client',
  ...
  try{
    ...
    if(m.role==='technicien'){
      ...
      if(!existingTech){
        const {error:techInsertError}=await sb
          .from('technicians')
          .insert({
            id:u.id,
