...
const serviceLabel={installation:'Installation',entretien:'Entretien',reparation:'Réparation'};
...
function globalClick(e){
  ...
  if(roleNav){e.preventDefault();const space=roleNav.dataset.space||profile?.role||'client';navigateSpace(space,roleNav.dataset.spaceNav);return}
...
async function createRequest(e){
  ...
  try{
    const serviceType=$('service')?.value||'installation';
    const payload={client_id:user.id,service_type:serviceType,contact_phone:$('phone')?.value.trim()||'',address:$('address')?.value.trim()||'',preferred_date:$('date')?.value?new Date($('date').value).toISOString():null,budget:$('budget')?.value?Number($('budget').value):null,description:$('description')?.value.trim()||'',status:'pending'};
...
function phoneForCall(phone){return String(phone||'').replace(/[^+\d]/g,'')}
function phoneForWhatsApp(phone){let n=String(phone||'').replace(/\D/g,'');if(n.startsWith('00221'))n=n.slice(2);if(n.startsWith('221'))return n;if(n.startsWith('0'))return '221'+n.slice(1);return n?'221'+n:n}
function contactButtons(phone,label){if(!phone)return '';const tel=phoneForCall(phone),wa=phoneForWhatsApp(phone);return `<div class="contactActions"><a class="secondaryBtn smallBtn" href="tel:${esc(tel)}">📞 Appeler</a><a class="secondaryBtn smallBtn" target="_blank" rel="noopener" href="https://wa.me/${esc(wa)}">💬 WhatsApp</a><span class="contactLabel">${esc(label)}</span></div>`}
async function loadContact(requestId){const {data,error}=await sb.rpc('get_request_contacts',{p_request_id:requestId});if(error)return alert(error.message);const c=data?.[0];if(!c?.other_phone)return alert('Aucun numéro de téléphone disponible.');const host=document.querySelector(`[data-contact-host="${CSS.escape(requestId)}"]`);if(host)host.innerHTML=contactButtons(c.other_phone,c.other_role==='client'?'Contact client':'Contact technicien')}
...
function techMissionRow(x){const iv=techState.interventions.find(v=>v.request_id===x.id);let action='';if(x.status==='accepted')action=`<button class="primary smallBtn" data-action="quote" data-id="${x.id}">Faire un devis</button>`;else if(x.status==='quoted')action='<span class="infoInline">Devis envoyé · en attente du client</span>';else if(x.status==='scheduled'&&iv)action=`<button class="primary smallBtn" data-action="start-intervention" data-id="${iv.id}">Démarrer</button>`;else if(x.status==='in_progress'&&iv)action=`<button class="primary smallBtn" data-action="complete-intervention" data-id="${iv.id}">Terminer</button>`;else if(x.status==='completed')action='<span class="infoInline">Mission terminée</span>';const contact=['accepted','quoted','scheduled','in_progress','completed'].includes(x.status)?`<div class="contactHost" data-contact-host="${esc(x.id)}"><button class="secondaryBtn smallBtn" data-action="contact" data-id="${x.id}">📞 Contacter le client</button></div>`:'';return `<div class="listRow missionRow"><div><strong>${esc(serviceLabel[x.service_type]||x.service_type)}</strong><small>${esc(x.address||'')} · ${fmtDateTime(x.preferred_date)}</small></div>${badge(x.status)}<div>${action}${contact}</div></div>`}
function clientNav(key,label){return `<button class="sideLink ${activeClient===key?'active':''}" data-space-nav="${key}" data-space="client">${label}</button>`}
function renderClientShell(page){
  const unread=clientState.notifications.filter(x=>!x.is_read).length;
  $('dashContent').innerHTML=`<div class="appShell clientShell"><aside id="-client" class="appSide clientSide"><div class="sideBrand"><b>Clim<span>Express</span></b><small>ESPACE CLIENT</small></div><nav class="sideNav">${clientNav('dashboard','Tableau de bord')} ${clientNav('reservations','Mes réservations')} ${clientNav('interventions','Mes interventions')} ${clientNav('devices','Mes appareils')} ${clientNav('profile','Mon profil')} ${clientNav('notifications',`Notifications${unread?` <em>${unread}</em>`:''}`)} ${clientNav('support','Support')}</nav><button class="logoutLink" data-action="logout">Déconnexion</button></aside><div id="overlay-client" class="drawerOverlay" data-close-drawer="client"></div><section class="appMain"><header class="appHeader"><button class="hamburger" data-drawer="client" aria-label="Ouvrir le menu"><i></i><i></i><i></i></button><b class="mobileBrand">Clim<span>Express</span></b><div class="headerSpacer"></div><button class="iconBtn" data-space-nav="notifications" data-space="client" aria-label="Notifications">🔔${unread?`<sup>${unread}</sup>`:''}</button><button class="profileChip" data-space-nav="profile" data-space="client"><span>${initials()}</span><strong>${esc(fullName())}</strong></button></header><main class="appContent">${clientPage(page)}</main></section></div>`;
  bindDynamicForms();
}
async function loadTech(){
  const t=await sb.from('technicians').select('*').eq('id',user.id).maybeSingle();
  if(t.error)throw t.error; techProfile=t.data;
  if(!techProfile)throw new Error('Profil technicien introuvable.');
  const [rq,mq,iv,nf]=await Promise.all([
    sb.from('requests').select('id,client_id,service_type,description,address,preferred_date,budget,status,technician_id,created_at').is('technician_id',null).eq('status','pending').order('created_at',{ascending:false}),
    sb.from('requests').select('id,client_id,service_type,description,address,preferred_date,budget,status,technician_id,created_at').eq('technician_id',user.id).order('created_at',{ascending:false}),
    sb.from('interventions').select('*').eq('technician_id',user.id).order('created_at',{ascending:false}),
    sb.from('notifications').select('*').eq('user_id',user.id).order('created_at',{ascending:false}).limit(20)
  ]);
  if(rq.error)throw rq.error;if(mq.error)throw mq.error;if(iv.error)throw iv.error;if(nf.error)throw nf.error;
...
function techNav(key,label){return `<button class="sideLink ${activeTech===key?'active':''}" data-space-nav="${key}" data-space="technicien">${label}</button>`}
...
async function createQuote(id){const amount=Number(prompt('Montant du devis en FCFA'));if(!Number.isFinite(amount)||amount<=0)return;const details=prompt('Détails du devis')||'';const {data,error}=await sb.rpc('technician_submit_quote',{p_request_id:id,p_amount:amount,p_details:details});if(error)return alert(error.message);if(!data)return alert('Le devis n’a pas pu être envoyé.');alert('✓ Devis envoyé au client.');await renderSpace()}
...
async function schedule(q,r){const el=$('dt-'+r);if(!el?.value)return alert('Choisissez une date et une heure.');const scheduled=new Date(el.value);if(Number.isNaN(scheduled.getTime()))return alert('Date invalide.');const {data,error}=await sb.rpc('schedule_intervention',{p_quote_id:q,p_request_id:r,p_scheduled_at:scheduled.toISOString()});if(error)return alert(error.message);if(!data)return alert('Le rendez-vous n’a pas pu être enregistré.');alert('✓ Rendez-vous enregistré.');await renderSpace()}
async function editProfile(){let specialty=prompt('Spécialité',techProfile.specialty||'');if(specialty===null)return;const {error}=await sb.from('technicians').update({specialty}).eq('id',user.id);if(error)return alert(error.message);await renderSpace()}
...
async function handleAction(el){
  ...
  try{
    ...
    if(a==='contact'){await loadContact(el.dataset.id);return}
...
async function init(){
  ...
  try{
    ...
    if(!user){document.body.classList.remove('client-mode','technician-mode','admin-mode');const header=$('publicHeader');header?.classList.remove('loggedIn');$('authBtn')?.classList.remove('hidden');$('userMenu')?.classList.add('hidden');$('dashboard')?.classList.add('hidden');return}
